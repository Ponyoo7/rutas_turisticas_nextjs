...
export const AddToRouteMap = ({ places, coords }: Props) => {
    ...
    const getDistance = (p1: OSMElement, p2: OSMElement) => {
        ...
        // Haversine formula for more accurate distance in km
        ...
        <div className="flex flex-col gap-4">
            <div className="flex justify-between items-end">
                <div className="flex flex-col gap-1">
                    <h2 className="text-xl font-bold">Planifica tu ruta</h2>
                    {routePlaces.length > 1 && (
                        <div className="flex items-center gap-4 text-sm text-slate-600">
                            ...
                            <span className="flex items-center gap-1">
                                ⏱️ {time >= 60 ? `${Math.floor(time / 60)}h ${time % 60}min` : `${time} min`}
                            </span>
                        </div>
                    )}
                </div>
            ...
            <MapWrapper places={places} coords={coords} onClick={addPlaceToRoute} routePlaces={routePlaces} />

            <Button onClick={handleSave}>Guardar</Button>
            <div className="flex flex-wrap gap-2">
                {routePlaces.map((r, i) => (
                    <PlaceCard key={r.id} place={r} index={i + 1} onDelete={removePlace} />
                ))}
            </div>
        </div>
    )
